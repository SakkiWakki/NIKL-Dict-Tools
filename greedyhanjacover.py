...
hanja2hangulpath = 'hanjareadings.json'
...
def greedy_hanja_match(s, data):
    ...
    for i in range(len(s) + 1):
        # print(f"On iter {i}")
        if (i != len(s) and cumstr in data) or not cumstr:
            # Match exists
            # print(f"Matched cum: {cumstr}")
            c = s[i]
            cumstr += c 
            continue
        
        substr = cumstr[:i-1] if i != len(s) else cumstr
        # print(f"Sub: {substr}")
        if substr in data: 
            # print(f"Matched sub: {substr}")
            ret += substr + LDELIM + data[substr] + RDELIM
            cumstr = cumstr[i-1:]
            # print(f"New cum: {cumstr}")
            if i == len(s):
                return ret
            c = s[i]
            cumstr += c 
